...
import 'package:aqua/data/provider/profile_settings_provider.dart';
...
class ProfilePasscodeConfirm extends StatelessWidget {
  static const routeName = '/profilePasscodeConfirm';

  const ProfilePasscodeConfirm({Key? key}) : super(key: key);
    ...
    return Stack(
      children: [
          ...
          onChange: (context, arguments) {
            if (arguments != null) {
              Navigator.of(context).pushNamedAndRemoveUntil(
                ProfilePasscodeConfirm.routeName,
                (route) =>
                    route is! RawDialogRoute &&
                    route.settings.name != ProfilePasscodeConfirm.routeName,
                arguments: arguments,
              );
            }
          },
        ...
        ProviderListener<String?>(
          provider: confirmPasscodeEnablePasscodeProvider(arguments),
          onChange: (context, passcode) {
            if (passcode != null) {
              context.read(profileSettingsProvider).enablePasscode(passcode);
            }
          },
          child: Container(),
        ),
