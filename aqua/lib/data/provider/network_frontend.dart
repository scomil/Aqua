...
abstract class NetworkFrontend with Frontend<GdkBackendEvent> {
  ...
  Future<Map<String, GdkAssetInformation>?> refreshAssets(
      {bool requiresRefresh = false}) async {
    if (requiresRefresh || allRawAssets == null || allRawAssets!.isEmpty) {
      allRawAssets = await _refreshAssets();
      // Workaround for breaking change in GDK
      if (allRawAssets != null) {
        final oldAsset = allRawAssets![policyAsset];
        allRawAssets![policyAsset] = GdkAssetInformation(
          assetId: policyAsset,
          name: 'Liquid Bitcoin',
          ticker: 'L-BTC',
          icon: oldAsset?.icon,
  ...
  Future<Map<String, GdkAssetInformation>?> _refreshAssets({
    GdkAssetsParameters params = const GdkAssetsParameters(),
  }) async {
    logger.d(params);
    final result = await runBackendMethod<GdkAssetsParameters,
  ...
  Future<GdkWallet?> getSubaccount({int subaccount = 0}) async {
    final result = await runBackendMethod<int, Result<GdkAuthHandlerStatus>>(
  ...
  Future<Tuple2<List<GdkPreviousAddress>, int?>?> getPreviousAddresses({
    ...
    return Tuple2(list, lastPointer);
  }
  ...
  Future<List<GdkPreviousAddress>> getAllPreviousAddresses() async {
    ...
    do {
      ...
      if (result?.item1 != null) {
        fullList.addAll(result!.item1);
      }
      lastPointer = result?.item2;
  ...
  Future<void> setTransactionMemo(String txhash, String memo) async {
    final tuple = Tuple2(txhash, memo);
    final result = await runBackendMethod<Tuple2<String, String>, Result<void>>(
...
class GdkNetworkException implements Exception {
  final Object error;
