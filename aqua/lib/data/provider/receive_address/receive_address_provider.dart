...
final receiveAddressProvider = Provider.family
    .autoDispose<ReceiveAddressProvider, Object?>(
...
class ReceiveAddressProvider {
  ...
  late final Stream<AsyncValue<Tuple2<Asset, List<GdkTransaction>>>>
      _dataStream = _reloadSubject
          .startWith(null)
          .switchMap((_) => Stream.value(_)
              ...
              .switchMap((asset) => Stream.value(asset)
                  ...
                  .map((transactions) => transactions
                      ...
                      .toList())
                  .map((transactions) => transactions ?? [])
                  .map((transactions) => Tuple2(asset, transactions)))
  ...
  late final Stream<AsyncValue<Tuple2<Asset, List<UsedAdresses>>>>
      _usedAddressesStream = _dataStream
          .switchMap((value) => value.when(
                data: (item) {
                  return Stream.value(item.item1).switchMap((asset) =>
                      Stream.value(item.item2).switchMap((transactions) =>
                          Stream.value(null).asyncMap((_) async {
                            ...
                            (allAddresses) => Stream.value(allAddresses)
                                .map((_) {
                                  ...
                                  return Tuple2(asset, usedAddresses);
                                })
                ...
                loading: () {
                  return Stream.value(const AsyncValue<
                      Tuple2<Asset, List<UsedAdresses>>>.loading());
                },
                error: (error, stackTrace) => Stream.value(
                    AsyncValue<Tuple2<Asset, List<UsedAdresses>>>.error(
                        error, stackTrace)),
              ))
          .shareReplay(maxSize: 1);
  ...
  Stream<ReceiveAddressUiModel> _usedAddressUiModelStream() =>
      _usedAddressesStream.asyncMap((value) => value.when(
          data: (data) => Stream.fromIterable(data.item2)
              .asyncMap((usedAddress) async {
                final asset = data.item1;
                ...
                final amount = usedAddress.transaction.satoshi == null
                    ? 0
                    : usedAddress.transaction.satoshi!
                                .containsKey(asset.assetId) ==
                            true
                        ? usedAddress.transaction.satoshi![asset.assetId] as int
                        : 0;
  ...
  late final Stream<AsyncValue<List<GdkPreviousAddress>>> _allAddressesStream =
      _dataStream.switchMap((value) => value.when(
            data: (item) => Stream.value(item.item1)
...
final _receiveUsedAddressesUiModelStreamProvider = StreamProvider.family
    .autoDispose<ReceiveAddressUiModel, Object?>((ref, arguments) async* {
  yield* ref
...
final receiveAddressesUiModelProvider = Provider.family
    .autoDispose<ReceiveAddressUiModel?, Object?>((ref, arguments) {
  return ref
