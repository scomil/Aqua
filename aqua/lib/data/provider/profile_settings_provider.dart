...
class ProfileSettingsProvider {
  ...
  late final Stream<AsyncValue<void>> _removeWalletAcceptStream = Rx.merge([
    _removeWalletAcceptSubject.switchMap((_) => Stream.value(_)
        ...
        .switchMap((biometricEnabled) => biometricEnabled
            ? Stream.value(null)
                .asyncMap((_) => _ref.read(aquaProvider).getMnemonic())
                .map((_) => Object())
    ...
    return Stream.value(_)
        .asyncMap((_) async => await _ref.read(aquaProvider).disconnect())
        .asyncMap((_) async => await _ref.read(aquaProvider).clearStorage())
        .doOnData((_) {
          _ref.read(aquaProvider).authorize();
        })
        .switchMap((_) => _ref.read(aquaProvider).authStream)
  ...
  late final Stream<AsyncValue<void>> _biometricProccessingStream = _switchBiometricSubject
      .switchMap((_) => Stream.value(null)
          .asyncMap((event) => _ref.read(aquaProvider).biometricEnabled())
          .map((_) => true)
          .onErrorReturn(false)
          .asyncMap((enabled) => enabled
              ? Stream.value(null)
                  .asyncMap(
                      (mnemonic) => _ref.read(aquaProvider).disableBiometric())
                  .first
              : Stream.value(null)
                  .asyncMap((_) => _ref.read(aquaProvider).passcodeEnabled())
                  .map((_) => true)
              ...
              _passcodeProccessingStream,
                ...
                .switchMap((value) => value.maybeWhen(
                      ...
                      orElse: () => Stream.value(null)
                          .asyncMap(
                              (_) => _ref.read(aquaProvider).biometricEnabled())
                          .map((_) => true)
                          .onErrorReturn(false),
                    ))
  ...
  late final Stream<AsyncValue<String>> _fallbackMnemonicStream =
      _switchPasscodeSubject
          .switchMap((_) => Stream.value(_)
              .asyncMap((event) => _ref.read(aquaProvider).getMnemonic())
  ...
  Stream<Object> _showPasscodeLockStream() => Rx.merge([
        ...
        _handlePasscodeUnlockResultSubject.switchMap((result) =>
          ...
          .asyncMap((event) => _ref.read(aquaProvider).passcodeEnabled())
          .map((_) => Object())
          ...
          .asyncMap((enabled) => enabled
              ? _handlePasscodeUnlockResultSubject
                  .switchMap((result) => result is PasscodeUnlockSuccessResult
                      ...
                      : const Stream<void>.empty())
                  .asyncMap(
                      (event) => _ref.read(aquaProvider).disablePasscode())
                  .first
            ...
            data: (initiallyEnabled) => Rx.merge([
              ...
              _passcodeProccessingStream,
                ...
                .switchMap((value) => value.maybeWhen(
                      ...
                      orElse: () => Stream.value(null)
                          .asyncMap(
                              (_) => _ref.read(aquaProvider).passcodeEnabled())
                          .map((_) => true)
                          .onErrorReturn(false),
                    ))
                .startWith(initiallyEnabled),
