...
class _WalletBackupConfirmationProvider {
  ...
  final PublishSubject<void> _skipInviteSubject = PublishSubject();
  void skipPrompt() {
    _skipInviteSubject.add(null);
  }

  Stream<Object> _popInviteStream() =>
      _skipInviteSubject.switchMap((_) => Stream.value(Object()));
      ...
      _confirmSubject
          .switchMap((_) => Stream.value(null)
              .asyncMap((_) async {
                ...
                await _ref.read(homeProvider).dismissRememberBackup();
...
final _walletBackupPopInviteStreamProvider =
    StreamProvider.autoDispose<Object>((ref) async* {
  yield* ref.watch(walletBackupConfirmationProvider)._popInviteStream();
});

final walletBackupPopInviteProvider = Provider.autoDispose<Object?>((ref) {
  return ref.watch(_walletBackupPopInviteStreamProvider).data?.value;
});
...
final walletBackupConfirmationResultProvider =
  ...
  return ref.watch(_walletBackupConfirmationResultStreamProvider).data?.value;
});
